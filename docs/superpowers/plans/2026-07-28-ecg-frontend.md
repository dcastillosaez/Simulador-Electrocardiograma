...
test("una sesion larga no degrada fps ni acumula memoria sin limite", async ({ page, baseURL }) => {
  ...
  try {
    ...
    // verificado rompiendo a propósito el descarte del buffer circular
    // (comentando el `while` de `FrameBuffer.push`) y confirmando que este
    // test seguía en verde con miles de frames retenidos sin evictar
    // ninguno. `ArrayBufferContents`, de la misma llamada CDP, sí crece con
    // esa fuga — es la señal que de verdad hay que vigilar.
